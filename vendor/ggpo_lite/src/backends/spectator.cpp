   ...
   _next_input_to_send(0)
{
   ...
   _host.Init( _callbacks, 0, {}, peer, NULL ); //TODO(DECLAN): Pass local
   _host.Synchronize();
...
GGPOErrorCode SpectatorBackend::DoPoll( GGPOMessage * msgs, int num ) {
    for( int i = 0; i < num; i++ ) {
        GGPOMessage msg = msgs[ i ];
        OnMsg( msg.peer, (UdpMsg *)msg.buffer, msg.len );
    }

    _host.OnLoopPoll( NULL );
...
void SpectatorBackend::OnMsg(GGPOPeerHandle from, UdpMsg *msg, int len) {
   if (_host.HandlesMsg(from)) {
      _host.OnMsg(msg, len);
   }
}
