...
Peer2PeerBackend::Peer2PeerBackend( GGPOSessionCallbacks * cb, const char * gamename, int num_players, int input_size, GGPOPeerHandle local ) :
    local(local),
...
void Peer2PeerBackend::AddRemotePlayer( GGPOPeerHandle peer, int queue ) {
    _synchronizing = true;

    _endpoints[ queue ].Init( _callbacks, queue, local, peer, _local_connect_status );
    _endpoints[ queue ].SetDisconnectTimeout( _disconnect_timeout );
    _endpoints[ queue ].SetDisconnectNotifyStart( _disconnect_notify_start );
    _endpoints[ queue ].Synchronize();
}

GGPOErrorCode Peer2PeerBackend::AddSpectator( GGPOPeerHandle peer ) {
    ...
    _spectators[ queue ].Init( _callbacks, queue + 1000, local, peer, _local_connect_status );
...
GGPOErrorCode Peer2PeerBackend::DoPoll( GGPOMessage * msgs, int num ) {
    if( !_sync.InRollback() ) {

        for( int i = 0; i < num; i++ ) {
            GGPOMessage msg = msgs[ i ];
            OnMsg( msg.peer, (UdpMsg *)msg.buffer, msg.len );
        }


        for( int i = 0; i < _num_players; i++ ) {
            _endpoints[ i ].OnLoopPoll(NULL);
        }
        ...
        if( !_synchronizing ) {
            ...
            if( current_frame > _next_recommended_sleep ) {
                ...
                if( interval > 0 ) {
                    Log( "Sending timesync notification to client.  interval=%d.\n", interval );
                    GGPOEvent info;
...
GGPOErrorCode Peer2PeerBackend::AddPlayer( GGPOPlayer * player, GGPOPlayerHandle * handle ) {
    if( player->type == GGPO_PLAYERTYPE_SPECTATOR ) {
        return AddSpectator( player->peer );
    }
    ...
    *handle = QueueToPlayerHandle( queue );
    ...
    if( player->type == GGPO_PLAYERTYPE_REMOTE ) {
        AddRemotePlayer( player->peer, queue );
    }
    return GGPO_OK;
}
    ...
    int size )
{
    ...
    input.Initialize( -1, (char *)values, size );
...
void Peer2PeerBackend::OnMsg( GGPOPeerHandle from, UdpMsg * msg, int len )
{
    for( int i = 0; i < _num_players; i++ ) {
        if( _endpoints[ i ].HandlesMsg( from ) ) {
            _endpoints[ i ].OnMsg( msg, len );
            return;
        }
    }
    for( int i = 0; i < _num_spectators; i++ ) {
        if( _spectators[ i ].HandlesMsg( from ) ) {
