...
{
    SetLogCallback( cb->log_msg );
    *session = ( GGPOSession * )new Peer2PeerBackend( cb, game, num_players, input_size, local );
    return GGPO_OK;
}

GGPOErrorCode ggpo_add_player( GGPOSession * ggpo, GGPOPlayer * player, GGPOPlayerHandle * handle ) {
    if( !ggpo ) {
...
GGPOErrorCode ggpo_start_synctest( GGPOSession ** ggpo, GGPOSessionCallbacks * cb, char * game, int num_players, int input_size, int frames ) {
    SetLogCallback( cb->log_msg );
    *ggpo = ( GGPOSession * )new SyncTestBackend( cb, game, frames, num_players );
    return GGPO_OK;
}

GGPOErrorCode ggpo_set_frame_delay( GGPOSession * ggpo, GGPOPlayerHandle player, int frame_delay ) {
    if( !ggpo ) {
        return GGPO_ERRORCODE_INVALID_SESSION;
    }
...
GGPOErrorCode ggpo_idle( GGPOSession * ggpo, GGPOMessage * msgs, int num ) {
    if( !ggpo ) {
        return GGPO_ERRORCODE_INVALID_SESSION;
    }
    return ggpo->DoPoll(msgs, num);
}
