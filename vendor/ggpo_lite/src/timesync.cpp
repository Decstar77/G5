...
void TimeSync::AdvanceFrame( GameInput & input, int advantage, int radvantage ) {
    // Remember the last frame and frame advantage
    last_inputs[ input.frame % ARRAY_SIZE( last_inputs ) ] = input;
    local[ input.frame % ARRAY_SIZE( local ) ] = advantage;
    remote[ input.frame % ARRAY_SIZE( remote ) ] = radvantage;
}

int TimeSync::RecommendFrameWaitDuration( bool require_idle_input ) {
    ...
    advantage = sum / (float)ARRAY_SIZE( local );

    sum = 0;
    for( i = 0; i < ARRAY_SIZE( remote ); i++ ) {
        sum += remote[ i ];
    }
    radvantage = sum / (float)ARRAY_SIZE( remote );
    ...
    if( require_idle_input ) {
        for( i = 1; i < ARRAY_SIZE( last_inputs ); i++ ) {
