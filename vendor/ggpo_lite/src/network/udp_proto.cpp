...
UdpProtocol::UdpProtocol() :
    ...
    initialized( false )
{
    _last_sent_input.Initialize( -1, NULL, 1 );
    _last_received_input.Initialize( -1, NULL, 1 );
    _last_acked_input.Initialize( -1, NULL, 1 );
...
void UdpProtocol::Init( GGPOSessionCallbacks callbacks, int queue, GGPOPeerHandle inLocal, GGPOPeerHandle inPeer, UdpMsg::connect_status * status ) {
    initialized = true;
    _queue = queue;
...
UdpProtocol::SendInput( GameInput & input )
{
    if( initialized ) {
        if( _current_state == Running ) {
             ...
             */
            _timesync.AdvanceFrame( input, _local_frame_advantage, _remote_frame_advantage );
...
UdpProtocol::SendPendingOutput()
{
    ...
    if( _pending_output.GetCount() ) {
        last = _last_acked_input;
        bits = msg->u.input.bits;

        msg->u.input.start_frame = _pending_output.GetFront().frame;
        msg->u.input.input_size = (uint8)_pending_output.GetFront().size;

        ASSERT( last.frame == -1 || last.frame + 1 == msg->u.input.start_frame );
        for( j = 0; j < _pending_output.GetCount(); j++ ) {
            GameInput & current = _pending_output.GetItem( j );
            if( memcmp( current.bits, last.bits, current.size ) != 0 ) {
                ...
                for( i = 0; i < current.size * 8; i++ ) {
                    ...
                    if( current.Value( i ) != last.Value( i ) ) {
                        BitVectorSetBit( msg->u.input.bits, &offset );
                        ( current.Value( i ) ? BitVectorSetBit : BitVectorClearBit )( bits, &offset );
                        BitVectorWriteNibblet( bits, i, &offset );
                    }
                }
            }
            BitVectorClearBit( msg->u.input.bits, &offset );
...
UdpProtocol::SendMsg( UdpMsg * msg )
{
    ...
    _send_queue.Push( QueueEntry( Platform::GetCurrentTimeMS(), peer, msg ) );
    PumpSendQueue();
}

bool UdpProtocol::HandlesMsg( GGPOPeerHandle from ) {
    if( !initialized ) {
        return false;
    }
...
UdpProtocol::OnInput( UdpMsg * msg, int len )
{
    ...
    if( msg->u.input.num_bits ) {
        ...
        while( offset < numBits ) {
            ...
            while( BitVectorReadBit( bits, &offset ) ) {
                int on = BitVectorReadBit( bits, &offset );
                int button = BitVectorReadNibblet( bits, &offset );
                if( useInputs ) {
                    if( on ) {
     ...
     */
    while( _pending_output.GetCount() && _pending_output.GetFront().frame < msg->u.input.ack_frame ) {
        Log( "Throwing away pending output frame %d\n", _pending_output.GetFront().frame );
        _last_acked_input = _pending_output.GetFront();
        _pending_output.Pop();
    }
    return true;
}
...
UdpProtocol::OnInputAck( UdpMsg * msg, int len )
{
     ...
     * Get rid of our buffered input
     */
    while( _pending_output.GetCount() && _pending_output.GetFront().frame < msg->u.input_ack.ack_frame ) {
        Log( "Throwing away pending output frame %d\n", _pending_output.GetFront().frame );
        _last_acked_input = _pending_output.GetFront();
        _pending_output.Pop();
    }
...
UdpProtocol::GetNetworkStats( struct GGPONetworkStats * s )
{
    ...
    s->network.send_queue_len = _pending_output.GetCount();
...
{
    while( !_send_queue.Clear() ) {
        QueueEntry & entry = _send_queue.GetFront();
        ...
        if( _send_latency ) {
            ...
            if( Platform::GetCurrentTimeMS() < _send_queue.GetFront().queue_time + jitter ) {
                break;
            }
        }
        if( _oop_percent && !_oo_packet.msg && ( ( rand() % 100 ) < _oop_percent ) ) {
            ...
            callbacks.network_send_to( local, entry.dest_addr, (char *)entry.msg, entry.msg->PacketSize() );
            delete entry.msg;
        }
        _send_queue.Pop();
    }
    if( _oo_packet.msg && _oo_packet.send_time < Platform::GetCurrentTimeMS() ) {
        ...
        callbacks.network_send_to( local, _oo_packet.dest_addr, (char *)_oo_packet.msg, _oo_packet.msg->PacketSize() );
...
void UdpProtocol::ClearSendQueue() {
    while( !_send_queue.Clear() ) {
        delete _send_queue.GetFront().msg;
        _send_queue.Pop();
    }
}
